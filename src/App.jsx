...
const N=(v)=>{const n=parseInt(v);return isNaN(n)?0:n};
...
const mkEmp=()=>[
  {id:uid(),nama:"Ahmad Fauzi",jabatan:"Kepala Bagian",dep:"Rumah Tangga",gaji:3500000,status:"Aktif",telp:"0812-3456-7890",
   kasbon:500000, // sisa kasbon yg belum lunas
   saldo:200000, // saldo dari periode sebelumnya (+ = kelebihan, - = kekurangan)
   slip:{masukHari:26,lembur:350000,gajiPerhari:115000,tunjangan:500000,bonus:200000,potKasbon:250000,potMakan:150000,periode:curPer()},
   logKasbon:[{id:uid(),tgl:"5 Agustus 2026",jumlah:750000,ket:"Keperluan keluarga"},{id:uid(),tgl:"20 Juli 2026",jumlah:500000,ket:"Biaya pengobatan"}],
   riwayat:[{periode:"Agustus 2026",slip:{masukHari:25,gajiPerhari:115000,lembur:300000,tunjangan:500000,bonus:100000,potKasbon:500000,potMakan:125000},gajiBersih:3150000,saldoSblm:350000,totalDiterima:3500000,saldoAkhir:200000,tgl:"31 Agustus 2026"}]},
  {id:uid(),nama:"Siti Aminah",jabatan:"Staff Administrasi",dep:"Sekretariat",gaji:2800000,status:"Aktif",telp:"0813-5678-1234",kasbon:0,saldo:0,slip:{masukHari:24,lembur:200000,gajiPerhari:95000,tunjangan:400000,bonus:150000,potKasbon:0,potMakan:120000,periode:curPer()},logKasbon:[],riwayat:[]},
  {id:uid(),nama:"Budi Santoso",jabatan:"Teknisi",dep:"Sarpras",gaji:3000000,status:"Aktif",telp:"0857-9012-3456",kasbon:300000,saldo:-150000,slip:{masukHari:25,gajiPerhari:100000,lembur:450000,tunjangan:350000,bonus:0,potKasbon:150000,potMakan:125000,periode:curPer()},logKasbon:[{id:uid(),tgl:"10 Agustus 2026",jumlah:300000,ket:"Darurat"}],riwayat:[]},
  {id:uid(),nama:"Dewi Rahayu",jabatan:"Juru Masak",dep:"Konsumsi",gaji:2500000,status:"Aktif",telp:"0821-2345-6789",kasbon:0,saldo:100000,slip:{masukHari:26,gajiPerhari:80000,lembur:150000,tunjangan:300000,bonus:100000,potKasbon:0,potMakan:130000,periode:curPer()},logKasbon:[],riwayat:[]},
];
...
// ── PDF SLIP GAJI ──
function printSlip(emp,slipData){
  const s=slipData;const c=calcSlip(s);
  const saldoSblm=emp.saldo||0;
  const totalDiterima=c.gajiBersih+saldoSblm;
  const sisaKasbon=emp.kasbon-s.potKasbon;
  const now=new Date();
  const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Slip Gaji - ${emp.nama}</title>
<style>@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700&display=swap');*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Plus Jakarta Sans',sans-serif;color:#1a1a1a;padding:32px;font-size:12px;line-height:1.5;max-width:560px;margin:0 auto}
.hdr{text-align:center;margin-bottom:18px;padding-bottom:12px;border-bottom:3px solid #1a3c34}.hdr h1{font-size:16px;color:#1a3c34}.hdr .sub{font-size:11px;color:#888;margin-top:2px}
.info{display:flex;justify-content:space-between;margin-bottom:16px;padding:10px 14px;background:#f8faf9;border-radius:6px;font-size:11px;line-height:1.7}
.sec{font-size:12px;font-weight:700;color:#1a3c34;padding:4px 0;border-bottom:2px solid #e0e0e0;margin:12px 0 6px}
.row{display:flex;justify-content:space-between;padding:5px 0;font-size:11px}.row .v{font-weight:600;font-variant-numeric:tabular-nums}.row.sub{padding-left:16px;color:#888;font-size:10px}.row.sub .v{font-weight:400}
.tot{background:#f4f7f6;border-radius:6px;padding:8px 14px;margin:6px 0}.tot .row{font-weight:700;font-size:12px}
.grand{background:linear-gradient(135deg,#1a3c34,#2d6a5a);border-radius:8px;padding:12px 16px;margin:10px 0;color:#fff}.grand .row{font-size:14px;font-weight:700;color:#fff}
.saldo{border:2px solid #1a3c34;border-radius:8px;padding:12px 14px;margin:12px 0}
.sign{display:flex;justify-content:space-between;margin-top:36px;font-size:10px}.sign-b{text-align:center}.sign-l{margin-top:45px;border-bottom:1px solid #333;width:140px;display:inline-block}
.ft{margin-top:20px;padding-top:8px;border-top:1px solid #e8e8e8;font-size:9px;color:#999;text-align:center}
@media print{body{padding:16px}@page{margin:10mm;size:A5}}</style></head><body>
<div class="hdr"><h1>SLIP GAJI KARYAWAN</h1><div class="sub">Departemen Rumah Tangga · Periode: ${s.periode}</div></div>
<div class="info"><div><b>Nama:</b> ${emp.nama}<br><b>Jabatan:</b> ${emp.jabatan}<br><b>Departemen:</b> ${emp.dep}</div><div style="text-align:right"><b>Gaji Standar:</b> ${fmtRp(emp.gaji)}<br><b>Sisa Kasbon:</b> ${fmtRp(emp.kasbon)}<br><b>Cetak:</b> ${fmtTgl(now)}</div></div>

<div class="sec">A. PENDAPATAN</div>
<div class="row"><span>Masuk Hari</span><span class="v">${s.masukHari} hari × ${fmtRp(s.gajiPerhari)}</span></div>
<div class="row sub"><span>= Gaji Pokok</span><span class="v">${fmtRp(c.gajiPokok)}</span></div>
...
<div class="row"><span>Pot. Kasbon</span><span class="v">${fmtRp(s.potKasbon)}</span></div>
<div class="row sub"><span>Sisa kasbon setelah potong: ${fmtRp(Math.max(0,sisaKasbon))}</span><span></span></div>
...
<div class="row"><span>Gaji Bersih (Pendapatan − Potongan)</span><span class="v">${fmtRp(c.gajiBersih)}</span></div>
<div class="row" style="color:${saldoSblm>=0?'#27ae60':'#c0392b'}"><span>Saldo Periode Sebelumnya</span><span class="v">${saldoSblm>=0?'+ ':''} ${fmtRp(saldoSblm)}</span></div>

<div class="grand"><div class="row"><span>TOTAL DITERIMA</span><span class="v">${fmtRp(totalDiterima)}</span></div></div>

<div class="saldo">
<div style="font-size:11px;font-weight:700;color:#1a3c34;margin-bottom:4px">Selisih & Saldo Baru</div>
<div class="row"><span>Gaji Standar</span><span class="v">${fmtRp(emp.gaji)}</span></div>
<div class="row"><span>Total Diterima</span><span class="v">${fmtRp(totalDiterima)}</span></div>
<div class="row" style="font-weight:700;color:${totalDiterima-emp.gaji>=0?'#27ae60':'#c0392b'};border-top:1.5px solid #1a3c34;padding-top:6px;margin-top:4px"><span>Saldo Baru → Periode Berikutnya</span><span class="v">${(totalDiterima-emp.gaji)>=0?'+':''}${fmtRp(totalDiterima-emp.gaji)}</span></div>
</div>
...
<div class="ft">Saldo (+) kelebihan · Saldo (−) kekurangan — otomatis terbawa ke periode berikutnya</div>
...
${p.pencairan.map(pc=>{const ic=pc.status!=="Belum Cair";if(ic)run-=pc.jumlah;const bg=pc.status==="Belum Cair"?"#fff3e0":pc.status==="Sudah Cair"?"#e3f2fd":pc.status==="Sudah Dilaporkan"?"#ede7f6":"#e8f5e9";const fg=pc.status==="Belum Cair"?"#e67e22":pc.status==="Sudah Cair"?"#2980b9":pc.status==="Sudah Dilaporkan"?"#7c3aed":"#27ae60";return`<tr><td class="c">${pc.tahap}</td><td>${pc.ket||"-"}</td><td class="r">${fmtRp(pc.jumlah)}</td><td class="r" style="color:${ic?'#c0392b':'#aaa'}">${ic?'- '+fmtRp(pc.jumlah):'-'}</td><td class="r" style="font-weight:600">${ic?fmtRp(run):'-'}</td><td class="c">${pc.tgl||"-"}</td><td class="c"><span class="b" style="background:${bg};color:${fg}">${pc.status}</span></td></tr>`}).join("")}
...
// ══════ LOGIN ══════
function LoginPage({onLogin}){const[u,setU]=useState("");const[p,setP]=useState("");const[sh,setSh]=useState(false);const[err,setErr]=useState("");const[now,setNow]=useState(new Date());useEffect(()=>{const t=setInterval(()=>setNow(new Date()),1000);return()=>clearInterval(t)},[]);const go=()=>{const f=ACCTS.find(a=>a.username===u&&a.password===p);if(f)onLogin(f);else setErr("Username atau password salah")};
return(<div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",minHeight:"100vh",background:"linear-gradient(145deg,#0f2b24,#1a3c34 30%,#2d6a5a 70%,#3b8b7a)",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><style>{gCSS}</style><div style={{background:"#fff",borderRadius:16,padding:"32px 28px 24px",boxShadow:"0 20px 60px rgba(0,0,0,0.25)",width:"100%",maxWidth:380}}>
  ...
  <div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div><label style={{fontSize:11,fontWeight:600,color:"#555",display:"block",marginBottom:4}}>Username</label><input style={{padding:"10px 12px",fontSize:13,border:"1.5px solid #e0e0e0",borderRadius:9,background:"#fafbfc",width:"100%",fontFamily:"inherit"}} value={u} onChange={e=>{setU(e.target.value);setErr("")}} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Username" autoFocus/></div>
    <div><label style={{fontSize:11,fontWeight:600,color:"#555",display:"block",marginBottom:4}}>Password</label><input style={{padding:"10px 12px",fontSize:13,border:"1.5px solid #e0e0e0",borderRadius:9,background:"#fafbfc",width:"100%",fontFamily:"inherit"}} type={sh?"text":"password"} value={p} onChange={e=>{setP(e.target.value);setErr("")}} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Password"/></div>
...
function SlipDetail({emp,onBack,onUpdate}){
  const s=emp.slip||mkSlip();
  const[form,setForm]=useState({...s});
  const[slTab,setSlTab]=useState("current");
  const[kbForm,setKbForm]=useState({jumlah:"",ket:""});
  const c=calcSlip(form);
  const saldoSblm=emp.saldo||0;
  const totalDiterima=c.gajiBersih+saldoSblm;
  const newSaldo=totalDiterima-emp.gaji;
  const sisaKasbonAfter=Math.max(0,(emp.kasbon||0)-form.potKasbon);
  const sF=(k,v)=>setForm({...form,[k]:N(v)});
  const saved=JSON.stringify(form)===JSON.stringify(s);

  const saveSlip=()=>onUpdate({...emp,slip:{...form}});
  const tutupPeriode=()=>{
    const rec={periode:form.periode,slip:{...form},gajiBersih:c.gajiBersih,saldoSblm,totalDiterima,saldoAkhir:newSaldo,tgl:fmtShort(new Date())};
    onUpdate({...emp,slip:{...mkSlip(),gajiPerhari:form.gajiPerhari,tunjangan:form.tunjangan},saldo:newSaldo,kasbon:sisaKasbonAfter,riwayat:[rec,...(emp.riwayat||[])]});
    setForm({...mkSlip(),gajiPerhari:form.gajiPerhari,tunjangan:form.tunjangan});
    setSlTab("history");
  };
  const addKasbon=()=>{
    const j=N(kbForm.jumlah);if(j<=0)return;
    onUpdate({...emp,kasbon:(emp.kasbon||0)+j,logKasbon:[{id:uid(),tgl:fmtShort(new Date()),jumlah:j,ket:kbForm.ket},...(emp.logKasbon||[])]});
    setKbForm({jumlah:"",ket:""});
  };
  const riwayat=emp.riwayat||[];

  return(<div>
    {/* Header */}
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:14,flexWrap:"wrap"}}>
      <button style={S.btnGhost} onClick={onBack}>{I.back} Kembali</button>
      <div style={{flex:1,minWidth:160}}>
        <h3 style={{fontSize:16,fontWeight:700,color:"#1a3c34"}}>Slip Gaji — {emp.nama}</h3>
        <div style={{fontSize:11,color:"#888"}}>{emp.jabatan} · {emp.dep} · Standar: {fmtRp(emp.gaji)}</div>
      </div>
      {/* ══ TOMBOL CETAK PDF ══ */}
      <button onClick={()=>{saveSlip();printSlip({...emp,slip:form},form)}}
        style={{padding:"10px 20px",fontSize:14,fontWeight:700,border:"none",borderRadius:10,background:"linear-gradient(135deg,#b91c1c,#dc2626)",color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",gap:8,boxShadow:"0 2px 8px rgba(185,28,28,0.3)"}}>
        {I.print} CETAK SLIP GAJI
      </button>
    </div>

    {/* Saldo + Kasbon summary */}
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
      <div style={{...S.card,margin:0,borderLeft:`4px solid ${saldoSblm>=0?"#27ae60":"#c0392b"}`,paddingTop:12,paddingBottom:12}}>
        <div style={{fontSize:10,fontWeight:600,color:"#888"}}>Saldo Periode Sebelumnya</div>
        <div style={{fontSize:22,fontWeight:700,color:saldoSblm>=0?"#27ae60":"#c0392b",marginTop:2}}>{saldoSblm>=0?"+":""}{fmtRp(saldoSblm)}</div>
        <div style={{fontSize:10,color:"#aaa",marginTop:2}}>{saldoSblm>0?"Kelebihan, ditambahkan":saldoSblm<0?"Kekurangan, dikurangkan":"Bersih"}</div>
      </div>
      <div style={{...S.card,margin:0,borderLeft:"4px solid #e67e22",paddingTop:12,paddingBottom:12}}>
        <div style={{fontSize:10,fontWeight:600,color:"#888"}}>Sisa Kasbon Belum Lunas</div>
        <div style={{fontSize:22,fontWeight:700,color:emp.kasbon>0?"#e67e22":"#27ae60",marginTop:2}}>{fmtRp(emp.kasbon||0)}</div>
        <div style={{fontSize:10,color:"#aaa",marginTop:2}}>{emp.kasbon>0?"Dipotong dari gaji":"Tidak ada kasbon"}</div>
      </div>
    </div>

    {/* Tabs */}
    <div style={{display:"flex",gap:2,marginBottom:14}}>
      {[["current","Slip Saat Ini"],["kasbon","Kasbon"],["history",`Riwayat (${riwayat.length})`]].map(([id,lb])=>(
        <button key={id} onClick={()=>setSlTab(id)} style={{padding:"8px 16px",fontSize:12,fontWeight:slTab===id?600:500,border:"none",background:slTab===id?"#fff":"#e8e8e8",color:slTab===id?"#1a3c34":"#666",cursor:"pointer",borderRadius:"8px 8px 0 0",boxShadow:slTab===id?"0 -1px 4px rgba(0,0,0,0.04)":"none"}}>{lb}</button>
      ))}
    </div>
    ...
    {slTab==="current"&&(<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(280px,1fr))",gap:14}}>
      {/* Form */}
      <div style={S.card}>
        <div style={S.cardHead}>Data Gaji — {form.periode}</div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            <div><label style={S.lbl}>Masuk Hari</label><input style={S.input} type="number" value={form.masukHari||""} onChange={e=>sF("masukHari",e.target.value)}/></div>
            <div><label style={S.lbl}>Gaji/Hari (Rp)</label><input style={S.input} type="number" value={form.gajiPerhari||""} onChange={e=>sF("gajiPerhari",e.target.value)}/></div>
          </div>
          <div style={{background:"#f0faf7",borderRadius:7,padding:"7px 12px",display:"flex",justifyContent:"space-between",border:"1px dashed #3b6b5e",fontSize:12}}><span style={{color:"#888"}}>Gaji Pokok = {form.masukHari} × {fmtRp(form.gajiPerhari)}</span><span style={{fontWeight:700,color:"#1a3c34"}}>{fmtRp(c.gajiPokok)}</span></div>
          {[["Lembur","lembur"],["Tunjangan","tunjangan"],["Bonus","bonus"]].map(([l,k])=><div key={k}><label style={S.lbl}>{l} (Rp)</label><input style={S.input} type="number" value={form[k]||""} onChange={e=>sF(k,e.target.value)}/></div>)}
          <div style={{borderTop:"2px dashed #fca5a5",paddingTop:10}}>
            <div style={{fontSize:11,fontWeight:700,color:"#c0392b",marginBottom:8}}>POTONGAN</div>
            <div><label style={S.lbl}>Pot. Kasbon (Rp) — sisa: {fmtRp(emp.kasbon||0)}</label><input style={{...S.input,borderColor:"#fca5a5"}} type="number" value={form.potKasbon||""} onChange={e=>sF("potKasbon",e.target.value)} max={emp.kasbon||0}/></div>
            {form.potKasbon>(emp.kasbon||0)&&<div style={{fontSize:10,color:"#c0392b",marginTop:2}}>Melebihi sisa kasbon!</div>}
            <div style={{marginTop:8}}><label style={S.lbl}>Pot. Makan (Rp)</label><input style={{...S.input,borderColor:"#fca5a5"}} type="number" value={form.potMakan||""} onChange={e=>sF("potMakan",e.target.value)}/></div>
          </div>
          <div style={{display:"flex",gap:6,marginTop:4}}>
            <button style={{...S.btnPri,flex:1,justifyContent:"center",opacity:saved?.5:1}} onClick={saveSlip} disabled={saved}>{saved?"Tersimpan":"Simpan"}</button>
            <button style={{...S.btnPri,flex:1,justifyContent:"center",background:"#7c3aed"}} onClick={tutupPeriode}>Tutup Periode</button>
          </div>
        </div>
      </div>
      ...
      {/* Preview */}
      <div style={S.card}>
        <div style={S.cardHead}>Preview Slip</div>
        <div style={{background:"#f8faf9",borderRadius:8,padding:"14px 16px"}}>
          <div style={{textAlign:"center",marginBottom:10}}><div style={{fontSize:10,fontWeight:700,color:"#1a3c34",letterSpacing:"0.04em"}}>SLIP GAJI — {form.periode}</div><div style={{fontSize:11,color:"#555",marginTop:2}}>{emp.nama} · {emp.jabatan}</div></div>
          ...
          {[["Gaji Pokok",`${form.masukHari}hr × ${fmtRp(form.gajiPerhari)}`,c.gajiPokok],["Lembur","",form.lembur],["Tunjangan","",form.tunjangan],["Bonus","",form.bonus]].map(([l,d,v])=><div key={l} style={{display:"flex",justifyContent:"space-between",padding:"3px 0",fontSize:11}}><span style={{color:"#555"}}>{l} {d&&<span style={{color:"#aaa",fontSize:9}}>({d})</span>}</span><span style={{fontWeight:500}}>{fmtRp(v)}</span></div>)}
          ...
          <div style={{display:"flex",justifyContent:"space-between",padding:"3px 0",fontSize:11}}><span style={{color:"#555"}}>Kasbon</span><span style={{fontWeight:500}}>{fmtRp(form.potKasbon)}</span></div>
          <div style={{padding:"1px 0 3px 12px",fontSize:9,color:"#aaa"}}>Sisa kasbon setelah potong: {fmtRp(sisaKasbonAfter)}</div>
          <div style={{display:"flex",justifyContent:"space-between",padding:"3px 0",fontSize:11}}><span style={{color:"#555"}}>Pot. Makan</span><span style={{fontWeight:500}}>{fmtRp(form.potMakan)}</span></div>
          ...
          {/* PERHITUNGAN AKHIR */}
          <div style={{marginTop:10,border:"2px solid #1a3c34",borderRadius:8,padding:"10px 12px"}}>
            <div style={{fontSize:10,fontWeight:700,color:"#1a3c34",marginBottom:6}}>PERHITUNGAN AKHIR</div>
            ...
            <div style={{display:"flex",justifyContent:"space-between",padding:"3px 0",fontSize:12,color:saldoSblm>=0?"#27ae60":"#c0392b"}}><span>Saldo Sebelumnya</span><span style={{fontWeight:600}}>{saldoSblm>=0?"+":""} {fmtRp(saldoSblm)}</span></div>
            <div style={{height:2,background:"#1a3c34",margin:"6px 0"}}/>
            <div style={{display:"flex",justifyContent:"space-between",padding:"3px 0",fontSize:14,fontWeight:800,color:"#1a3c34"}}><span>TOTAL DITERIMA</span><span>{fmtRp(totalDiterima)}</span></div>
          </div>

          <div style={{background:newSaldo>=0?"#e8f5e9":"#fce4ec",borderRadius:6,padding:"8px 12px",marginTop:8,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <span style={{fontSize:10,fontWeight:600,color:newSaldo>=0?"#27ae60":"#c0392b"}}>Saldo → Periode Berikutnya</span>
            <span style={{fontSize:14,fontWeight:800,color:newSaldo>=0?"#27ae60":"#c0392b"}}>{newSaldo>=0?"+":""}{fmtRp(newSaldo)}</span>
          </div>
        </div>
      </div>
    </div>)}
    ...
    {slTab==="kasbon"&&(<div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
        <div style={{...S.card,margin:0,borderTop:"3px solid #e67e22"}}><div style={S.statLbl}>Sisa Kasbon</div><div style={{fontSize:24,fontWeight:700,color:"#e67e22"}}>{fmtRp(emp.kasbon||0)}</div></div>
        <div style={{...S.card,margin:0,borderTop:"3px solid #27ae60"}}><div style={S.statLbl}>Total Pernah Dipinjam</div><div style={{fontSize:24,fontWeight:700,color:"#1a3c34"}}>{fmtRp((emp.logKasbon||[]).reduce((s,x)=>s+x.jumlah,0))}</div></div>
      </div>
      <div style={S.card}><div style={S.cardHead}>Ambil Kasbon Baru</div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end"}}>
          <div style={{flex:1,minWidth:120}}><label style={S.lbl}>Jumlah (Rp)</label><input style={S.input} type="number" value={kbForm.jumlah} onChange={e=>setKbForm({...kbForm,jumlah:e.target.value})} placeholder="0"/></div>
          <div style={{flex:2,minWidth:150}}><label style={S.lbl}>Keterangan</label><input style={S.input} value={kbForm.ket} onChange={e=>setKbForm({...kbForm,ket:e.target.value})} placeholder="Keperluan"/></div>
          <button style={S.btnPri} onClick={addKasbon}>{I.plus} Ambil Kasbon</button>
        </div>
      </div>
      <div style={S.card}><div style={S.cardHead}>Riwayat Pengambilan Kasbon</div>
        {!(emp.logKasbon||[]).length?<div style={S.empty}>Belum ada</div>:
        <div style={{display:"flex",flexDirection:"column",gap:8}}>{(emp.logKasbon||[]).map(k=>(
          <div key={k.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:"#fafbfc",borderRadius:8,borderLeft:"3px solid #e67e22"}}>
            <div><div style={{fontWeight:500,fontSize:13}}>{fmtRp(k.jumlah)}</div><div style={{fontSize:11,color:"#999"}}>{k.tgl}{k.ket&&` · ${k.ket}`}</div></div>
          </div>
        ))}</div>}
      </div>
    </div>)}
    ...
    {/* ══ TAB: RIWAYAT ══ */}
    {slTab==="history"&&(<div style={S.card}>
      <div style={S.cardHead}>Riwayat Gaji</div>
      {!riwayat.length?<div style={S.empty}>Belum ada riwayat. Klik "Tutup Periode" untuk menyimpan.</div>:
      <div style={S.tableWrap}><table style={S.table}><thead><tr>
        <th style={{...S.th,textAlign:"left"}}>Periode</th>
        <th style={S.th}>Pendapatan</th>
        <th style={S.th}>Potongan</th>
        <th style={S.th}>Gaji Bersih</th>
        <th style={S.th}>Saldo Sblm</th>
        <th style={{...S.th,fontWeight:700}}>Total Diterima</th>
        <th style={S.th}>Saldo Akhir</th>
      </tr></thead><tbody>
        {riwayat.map((r,i)=>{const cs=calcSlip(r.slip);return(<tr key={i} style={i%2?{background:"#fafbfc"}:{}}>
          <td style={{...S.td,textAlign:"left"}}><div style={{fontWeight:600}}>{r.periode}</div><div style={{fontSize:10,color:"#aaa"}}>{r.tgl}</div></td>
          <td style={{...S.td,color:"#27ae60"}}>{fmtRp(cs.totalPendapatan)}</td>
          <td style={{...S.td,color:"#c0392b"}}>{fmtRp(cs.totalPotongan)}</td>
          <td style={{...S.td,fontWeight:600}}>{fmtRp(r.gajiBersih)}</td>
          <td style={{...S.td,color:r.saldoSblm>=0?"#27ae60":"#c0392b"}}>{r.saldoSblm>=0?"+":""}{fmtRp(r.saldoSblm)}</td>
          <td style={{...S.td,fontWeight:700,color:"#1a3c34"}}>{fmtRp(r.totalDiterima)}</td>
          <td style={{...S.td,fontWeight:700,color:r.saldoAkhir>=0?"#27ae60":"#c0392b"}}>{r.saldoAkhir>=0?"+":""}{fmtRp(r.saldoAkhir)}</td>
        </tr>)})}
      </tbody></table></div>}
    </div>)}
  </div>);
...
// ══════ MAIN ══════
function MainApp({account:acc,onLogout}){
  ...
  const[projects,setProjects]=useState(mkProj);const[employees,setEmployees]=useState(mkEmp);const[loans,setLoans]=useState([]);const[tx,setTx]=useState([]);
  ...
  const allPc=useMemo(()=>projects.flatMap(p=>p.pencairan.map(pc=>({...pc,pN:p.name,pI:p.id}))),[projects]);
  ...
  const Top=({t})=>(<header style={S.topbar}><div style={{display:"flex",alignItems:"center",gap:10}}><button style={S.menuBtn} onClick={()=>setSideOpen(true)}>{I.menu}</button><h2 style={S.pageTitle}>{t}</h2></div><div style={S.topClock}><span style={S.topTime}>{fmtJam(now)}</span><span style={S.topDate}>{fmtTgl(now)}</span></div></header>);
  ...
  <main style={S.main}>
    {tab==="dashboard"&&(<div>
      <div style={S.welcomeBar}><div style={{fontSize:16,fontWeight:600}}>Selamat datang, {acc.nama}</div><div style={{fontSize:11,opacity:.7,marginTop:2}}>Ringkasan Keuangan Dept. Rumah Tangga</div></div>
    ...
    {tab==="karyawan"&&(<div>
      ...
      {showEF&&<div style={S.card}><div style={S.cardHead}>{editE?"Edit":"Tambah"} Karyawan</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:8}}>{[["Nama","nama"],["Jabatan","jabatan"],["Dept","dep"],["Gaji Standar","gaji"],["Telp","telp"]].map(([l,k])=>(<div key={k}><label style={S.lbl}>{l}</label><input style={S.input} type={k==="gaji"?"number":"text"} value={eF[k]} onChange={e=>setEF({...eF,[k]:e.target.value})}/></div>))}<div><label style={S.lbl}>Status</label><select style={S.select} value={eF.status} onChange={e=>setEF({...eF,status:e.target.value})}><option>Aktif</option><option>Nonaktif</option><option>Cuti</option></select></div></div><div style={{display:"flex",gap:6,marginTop:12}}>{editE?<button style={S.btnPri} onClick={()=>{setEmployees(employees.map(e=>e.id===editE?{...e,...eF,gaji:N(eF.gaji)}:e));setEditE(null);setShowEF(false)}}>Simpan</button>:<button style={S.btnPri} onClick={()=>{if(!eF.nama||!eF.gaji)return;setEmployees([...employees,{id:uid(),...eF,gaji:N(eF.gaji),slip:mkSlip(),saldo:0,kasbon:0,logKasbon:[],riwayat:[]}]);setEF({nama:"",jabatan:"",dep:"",gaji:"",telp:"",status:"Aktif"});setShowEF(false)}}>{I.plus} Simpan</button>}<button style={S.btnSec} onClick={()=>{setShowEF(false);setEditE(null)}}>Batal</button></div></div>}
      <div style={S.card}><div style={S.cardHead}>Daftar Karyawan {empQ&&<span style={{fontWeight:400,color:"#aaa",fontSize:11}}>· {fEmp.length} hasil</span>}</div>
      {!fEmp.length?<div style={S.empty}>{empQ?"Tidak ditemukan":"Belum ada"}</div>:<div style={S.tableWrap}><table style={S.table}><thead><tr><th style={S.th}>No</th><th style={{...S.th,textAlign:"left"}}>Nama</th><th style={{...S.th,textAlign:"left"}}>Jabatan</th><th style={S.th}>Total Diterima</th><th style={S.th}>Kasbon</th><th style={S.th}>Saldo</th><th style={S.th}>Aksi</th></tr></thead><tbody>{fEmp.map((e,i)=>{const cs=calcSlip(e.slip||mkSlip());const td=cs.gajiBersih+(e.saldo||0);return(<tr key={e.id} style={i%2?{background:"#fafbfc"}:{}}><td style={S.td}>{i+1}</td><td style={{...S.td,textAlign:"left"}}><div style={{fontWeight:500}}>{e.nama}</div><div style={{fontSize:10,color:"#999"}}>{e.telp}</div></td><td style={{...S.td,textAlign:"left",fontSize:11}}>{e.jabatan}<br/><span style={{...S.badge,background:"#eef2ff",color:"#4338ca"}}>{e.dep}</span></td>
      <td style={{...S.td,fontWeight:700,color:"#1a3c34"}}>{fmtRp(td)}<br/><span style={{fontWeight:400,fontSize:9,color:"#999"}}>bersih {fmtRp(cs.gajiBersih)} {(e.saldo||0)!==0?`${(e.saldo||0)>0?'+':''}${fmtRp(e.saldo||0)}`:""}</span></td>
      <td style={{...S.td,fontWeight:600,color:e.kasbon?"#e67e22":"#aaa"}}>{fmtRp(e.kasbon||0)}</td>
      <td style={{...S.td,fontWeight:700,color:(e.saldo||0)>=0?"#27ae60":"#c0392b"}}>{(e.saldo||0)>=0?"+":""}{fmtRp(e.saldo||0)}</td>
      <td style={S.td}><div style={{display:"flex",gap:4,justifyContent:"center",flexWrap:"wrap"}}>
        <button style={{...S.btnMini,borderColor:"#1a3c34",color:"#1a3c34"}} onClick={()=>setSlipE(e.id)}>{I.slip} Slip</button>
        <button style={S.iconBtn} onClick={()=>{setEditE(e.id);setEF({nama:e.nama,jabatan:e.jabatan,dep:e.dep,gaji:e.gaji.toString(),telp:e.telp||"",status:e.status});setShowEF(true)}}>{I.edit}</button>
        <button style={{...S.iconBtn,color:"#c0392b"}} onClick={()=>setEmployees(employees.filter(x=>x.id!==e.id))}>{I.trash}</button>
      </div></td></tr>)})}</tbody></table></div>}</div>
    </div>)}

    {tab==="pinjaman"&&(<div>
      <div style={S.card}><div style={S.cardHead}>Pinjaman Antar Proyek</div>{projects.length<2?<div style={S.empty}>Perlu 2+ proyek</div>:<><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))",gap:8}}>{[["Dari","dari"],["Ke","ke"]].map(([l,k])=>(<div key={k}><label style={S.lbl}>{l}</label><select style={S.select} value={lF[k]} onChange={e=>setLF({...lF,[k]:e.target.value})}><option value="">— Pilih —</option>{projects.filter(p=>p.id!==(k==="dari"?lF.ke:lF.dari)).map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>))}<div><label style={S.lbl}>Jumlah</label><input style={S.input} type="number" value={lF.jumlah} onChange={e=>setLF({...lF,jumlah:e.target.value})}/></div><div><label style={S.lbl}>Ket</label><input style={S.input} value={lF.ket} onChange={e=>setLF({...lF,ket:e.target.value})}/></div></div><button style={{...S.btnPri,marginTop:12}} onClick={()=>{if(!lF.dari||!lF.ke||!lF.jumlah||lF.dari===lF.ke)return;setLoans([...loans,{id:uid(),dari:projects.find(p=>p.id===lF.dari).name,ke:projects.find(p=>p.id===lF.ke).name,jumlah:N(lF.jumlah),ket:lF.ket,tgl:fmtTgl(new Date())}]);setLF({dari:"",ke:"",jumlah:"",ket:""})}}>Buat</button></>}</div>
...
export default function App(){const[a,setA]=useState(null);return a?<MainApp account={a} onLogout={()=>setA(null)}/>:<LoginPage onLogin={setA}/>;}

const S={shell:{fontFamily:"'Plus Jakarta Sans',sans-serif",display:"flex",minHeight:"100vh",background:"#f0f2f5",color:"#1a1a1a"},side:{width:230,background:"linear-gradient(180deg,#0f2b24,#1a3c34)",color:"#fff",display:"flex",flexDirection:"column",position:"fixed",top:0,left:0,bottom:0,zIndex:100,transform:"translateX(-100%)",transition:"transform 0.25s ease"},sideOpen:{transform:"translateX(0)"},sideOver:{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:99},sideBrand:{padding:"16px 14px",display:"flex",alignItems:"center",gap:8,borderBottom:"1px solid rgba(255,255,255,0.1)"},sideLogo:{width:34,height:34,borderRadius:7,background:"rgba(255,255,255,0.12)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},sideBT:{fontSize:13,fontWeight:700},sideBS:{fontSize:9,opacity:.5},sideX:{marginLeft:"auto",background:"none",border:"none",color:"rgba(255,255,255,0.5)",cursor:"pointer",padding:4},sideNav:{flex:1,padding:"10px 6px",display:"flex",flexDirection:"column",gap:1},sideItem:{display:"flex",alignItems:"center",gap:9,padding:"9px 11px",borderRadius:7,border:"none",background:"transparent",color:"rgba(255,255,255,0.65)",cursor:"pointer",fontSize:12,fontWeight:500,textAlign:"left",width:"100%"},sideItemA:{background:"rgba(255,255,255,0.12)",color:"#fff",fontWeight:600},sideBot:{padding:"10px 10px 14px",borderTop:"1px solid rgba(255,255,255,0.1)"},sideUser:{display:"flex",alignItems:"center",gap:8,marginBottom:8},sideAv:{width:30,height:30,borderRadius:"50%",background:"rgba(255,255,255,0.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700},sideLogout:{display:"flex",alignItems:"center",gap:7,padding:"7px 10px",borderRadius:7,border:"none",background:"rgba(255,255,255,0.06)",color:"rgba(255,255,255,0.5)",cursor:"pointer",fontSize:11,width:"100%"},content:{flex:1,display:"flex",flexDirection:"column",minHeight:"100vh"},topbar:{background:"#fff",padding:"12px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",boxShadow:"0 1px 3px rgba(0,0,0,0.05)",position:"sticky",top:0,zIndex:50},menuBtn:{background:"none",border:"none",cursor:"pointer",color:"#1a3c34",padding:3,display:"flex"},pageTitle:{fontSize:16,fontWeight:700,color:"#1a3c34"},topClock:{display:"flex",flexDirection:"column",alignItems:"flex-end"},topTime:{fontSize:17,fontWeight:300,color:"#1a3c34",fontVariantNumeric:"tabular-nums"},topDate:{fontSize:10,color:"#999"},main:{padding:18,flex:1},welcomeBar:{background:"linear-gradient(135deg,#1a3c34,#2d6a5a)",borderRadius:11,padding:"18px 22px",color:"#fff",marginBottom:18},grid4:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginBottom:18},grid3:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10,marginBottom:14},grid2:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(250px,1fr))",gap:10,marginBottom:14},statCard:{background:"#fff",borderRadius:9,padding:"13px 15px",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"},statLbl:{fontSize:10,fontWeight:500,color:"#888",marginBottom:4},statVal:{fontSize:19,fontWeight:700,color:"#1a3c34",fontVariantNumeric:"tabular-nums"},statMeta:{fontSize:10,color:"#aaa",marginTop:3},card:{background:"#fff",borderRadius:11,padding:"16px 18px",boxShadow:"0 1px 3px rgba(0,0,0,0.04)",marginBottom:12},cardHead:{fontSize:13,fontWeight:700,color:"#1a3c34",marginBottom:12,paddingBottom:7,borderBottom:"1px solid #f0f0f0"},progressOuter:{height:5,background:"#eee",borderRadius:3,overflow:"hidden"},progressInner:{height:"100%",borderRadius:3,transition:"width 0.4s"},lbl:{fontSize:10,fontWeight:600,color:"#666",display:"block",marginBottom:3},input:{padding:"8px 11px",fontSize:12,border:"1.5px solid #e0e0e0",borderRadius:7,background:"#fafbfc",color:"#1a1a1a",width:"100%",fontFamily:"inherit"},select:{padding:"8px 11px",fontSize:12,border:"1.5px solid #e0e0e0",borderRadius:7,background:"#fafbfc",color:"#1a1a1a",cursor:"pointer",fontFamily:"inherit",width:"100%"},btnPri:{padding:"8px 15px",fontSize:12,fontWeight:600,border:"none",borderRadius:7,background:"#1a3c34",color:"#fff",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:5,whiteSpace:"nowrap"},btnSec:{padding:"8px 13px",fontSize:12,fontWeight:500,border:"1.5px solid #ccc",borderRadius:7,background:"#fff",color:"#555",cursor:"pointer"},btnGhost:{display:"flex",alignItems:"center",gap:4,padding:"6px 11px",fontSize:12,fontWeight:500,border:"1.5px solid #ddd",borderRadius:7,background:"#fff",color:"#1a3c34",cursor:"pointer"},btnRet:{display:"flex",alignItems:"center",gap:3,padding:"5px 9px",fontSize:11,fontWeight:500,border:"1.5px solid #3b6b5e",borderRadius:5,background:"#f0faf7",color:"#1a3c34",cursor:"pointer",whiteSpace:"nowrap"},btnMini:{padding:"3px 8px",fontSize:10,fontWeight:600,border:"1.5px solid",borderRadius:4,background:"#fff",cursor:"pointer",whiteSpace:"nowrap",display:"inline-flex",alignItems:"center",gap:2},iconBtn:{background:"none",border:"none",cursor:"pointer",color:"#666",padding:3,display:"flex",borderRadius:3},linkBtn:{background:"none",border:"none",cursor:"pointer",color:"#1a3c34",fontWeight:600,fontSize:12,textDecoration:"underline",textUnderlineOffset:2,padding:0,fontFamily:"inherit"},tableWrap:{overflowX:"auto"},table:{width:"100%",borderCollapse:"collapse",fontSize:11},th:{padding:"8px 9px",fontWeight:600,fontSize:9,color:"#888",borderBottom:"2px solid #f0f0f0",textAlign:"center",whiteSpace:"nowrap"},td:{padding:"8px 9px",borderBottom:"1px solid #f5f5f5",textAlign:"center",fontVariantNumeric:"tabular-nums"},badge:{padding:"2px 8px",borderRadius:16,fontSize:9,fontWeight:600,whiteSpace:"nowrap"},empty:{fontSize:12,color:"#bbb",fontStyle:"italic",padding:"16px 0",textAlign:"center"}};
