...
return(<div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",minHeight:"100vh",background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><style>{gCSS}</style><div style={{background:T.card,borderRadius:20,padding:"36px 30px 28px",boxShadow:"0 20px 60px rgba(0,0,0,0.5)",border:`1px solid ${T.cb}`,width:"100%",maxWidth:380}}>
  <div style={{textAlign:"center",marginBottom:18}}><div style={{width:64,height:64,borderRadius:14,background:T.iBg,display:"inline-flex",alignItems:"center",justifyContent:"center",marginBottom:10,border:`1px solid ${T.cb}`}}><img src={LOGO_IMG} style={{width:48,height:"auto"}} alt="DRT"/></div><h1 style={{fontSize:18,fontWeight:700,color:T.tx}}>Administrasi Keuangan</h1><p style={{fontSize:12,color:T.mut,marginTop:2}}>Departemen Rumah Tangga</p></div>
  <div style={{textAlign:"center",padding:"8px 0",marginBottom:12,borderTop:`1px solid ${T.cb}`,borderBottom:`1px solid ${T.cb}`}}><span style={{fontSize:20,fontWeight:300,color:T.sub,fontVariantNumeric:"tabular-nums"}}>{fmtJam(now)}</span><span style={{display:"block",fontSize:10,color:T.mut,marginTop:1}}>{fmtTgl(now)}</span></div>
  <div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div><label style={{fontSize:11,fontWeight:600,color:T.mut,display:"block",marginBottom:4}}>Username</label><input style={S.loginInput} value={u} onChange={e=>{setU(e.target.value);setErr("")}} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Username" autoFocus/></div>
    <div><label style={{fontSize:11,fontWeight:600,color:T.mut,display:"block",marginBottom:4}}>Password</label><input style={S.loginInput} type={sh?"text":"password"} value={p} onChange={e=>{setP(e.target.value);setErr("")}} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Password"/></div>
    ...
    <button style={{padding:12,fontSize:13,fontWeight:600,border:"none",borderRadius:10,background:T.gr,color:"#fff",cursor:"pointer",marginTop:2,boxShadow:`0 4px 12px ${T.acBg}`}} onClick={go}>Masuk</button>
  </div>
  <div style={{textAlign:"center",fontSize:10,color:T.mut,marginTop:16}}>admin / admin123 · bendahara / bend123</div>
</div></div>);}
...
function MainApp({account:acc,onLogout,appData,setAppData,onSave}){
  ...
  <main style={S.main}>
    {tab==="dashboard"&&(<div>
      <div style={{background:`linear-gradient(135deg,${T.sA},${T.sB})`,borderRadius:14,padding:"20px 24px",color:"#e0e0f0",marginBottom:18,border:`1px solid ${T.cb}`}}><div style={{fontSize:16,fontWeight:600}}>{L.selamat}, {acc.nama}</div><div style={{fontSize:11,opacity:.5,marginTop:2}}>{L.ringkasan} · {L.menyimpan.replace("...","")}{I.cloud}</div></div>
