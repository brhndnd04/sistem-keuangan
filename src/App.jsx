...
const gCSS=`@import url('https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@300;400;500;600;700;800&display=swap');*{box-sizing:border-box;margin:0;padding:0}input,select,button{font-family:inherit}input:focus,select:focus{outline:none;border-color:#6c63ff!important;box-shadow:0 0 0 3px rgba(108,99,255,0.15)}button:active{transform:scale(0.97)}::-webkit-scrollbar{width:5px}::-webkit-scrollbar-thumb{background:#444;border-radius:8px}::-webkit-scrollbar-track{background:#1a1a2e}body{background:#0f0f1a}`;
...
return(<div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",minHeight:"100vh",background:"#0f0f1a",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><style>{gCSS}</style><div style={{background:"#1a1a2e",borderRadius:20,padding:"36px 30px 28px",boxShadow:"0 20px 60px rgba(0,0,0,0.5),0 0 0 1px rgba(255,255,255,0.05)",width:"100%",maxWidth:380}}>
  <div style={{textAlign:"center",marginBottom:18}}><div style={{width:64,height:64,borderRadius:14,background:"rgba(255,255,255,0.08)",display:"inline-flex",alignItems:"center",justifyContent:"center",marginBottom:10,border:"1px solid rgba(255,255,255,0.08)"}}><img src={LOGO_IMG} style={{width:48,height:"auto"}} alt="DRT"/></div><h1 style={{fontSize:18,fontWeight:700,color:"#f0f0f0"}}>Administrasi Keuangan</h1><p style={{fontSize:12,color:"#666",marginTop:2}}>Departemen Rumah Tangga</p></div>
  <div style={{textAlign:"center",padding:"8px 0",marginBottom:12,borderTop:"1px solid rgba(255,255,255,0.06)",borderBottom:"1px solid rgba(255,255,255,0.06)"}}><span style={{fontSize:20,fontWeight:300,color:"#8b8baa",fontVariantNumeric:"tabular-nums"}}>{fmtJam(now)}</span><span style={{display:"block",fontSize:10,color:"#888",marginTop:1}}>{fmtTgl(now)}</span></div>
  <div style={{display:"flex",flexDirection:"column",gap:12}}>
    <div><label style={{fontSize:11,fontWeight:600,color:"#777",display:"block",marginBottom:4}}>Username</label><input style={S.loginInput} value={u} onChange={e=>{setU(e.target.value);setErr("")}} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Username" autoFocus/></div>
    <div><label style={{fontSize:11,fontWeight:600,color:"#777",display:"block",marginBottom:4}}>Password</label><input style={S.loginInput} type={sh?"text":"password"} value={p} onChange={e=>{setP(e.target.value);setErr("")}} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Password"/></div>
    {err&&<div style={{fontSize:11,color:"#ff6b6b",background:"rgba(255,107,107,0.1)",padding:"7px 10px",borderRadius:7,textAlign:"center"}}>{err}</div>}
    <button style={{padding:12,fontSize:13,fontWeight:600,border:"none",borderRadius:10,background:"linear-gradient(135deg,#6c63ff,#4834d4)",color:"#fff",cursor:"pointer",marginTop:2,boxShadow:"0 4px 12px rgba(108,99,255,0.3)"}} onClick={go}>Masuk</button>
  </div>
  <div style={{textAlign:"center",fontSize:10,color:"#444",marginTop:16}}>admin / admin123 · bendahara / bend123</div>
</div></div>);}
...
function SlipDetail({emp,onBack,onUpdate}){
  ...
  return(<div>
    ...
    {slTab==="history"&&(<div style={S.card}><div style={S.cardHead}>Riwayat Gaji</div>{!(emp.riwayat||[]).length?<div style={S.empty}>Belum ada. Klik "Tutup Periode" untuk simpan.</div>:<div style={S.tableWrap}><table style={S.table}><thead><tr><th style={{...S.th,textAlign:"left"}}>Periode</th><th style={S.th}>Gaji Bersih</th><th style={S.th}>Saldo Sblm</th><th style={S.th}>Harus Bayar</th><th style={{...S.th,fontWeight:700}}>Dibayarkan</th><th style={S.th}>Saldo Akhir</th></tr></thead><tbody>{(emp.riwayat||[]).map((r,i)=>{const hb=r.harusBayar||(r.gajiBersih+(r.saldoSblm||0));const db=r.dibayarkan||r.totalDiterima||0;return(<tr key={i} style={i%2?{background:"rgba(255,255,255,0.03)"}:{}}><td style={{...S.td,textAlign:"left"}}><div style={{fontWeight:600}}>{r.periode}</div><div style={{fontSize:10,color:"#666"}}>{r.tgl}</div></td><td style={{...S.td,fontWeight:600}}>{fmtRp(r.gajiBersih)}</td><td style={{...S.td,color:(r.saldoSblm||0)>=0?"#27ae60":"#c0392b"}}>{(r.saldoSblm||0)>=0?"+":""}{fmtRp(r.saldoSblm||0)}</td><td style={S.td}>{fmtRp(hb)}</td><td style={{...S.td,fontWeight:700,color:"#b8b2ff"}}>{fmtRp(db)}</td><td style={{...S.td,fontWeight:700,color:(r.saldoAkhir||0)>=0?"#27ae60":"#c0392b"}}>{(r.saldoAkhir||0)>=0?"+":""}{fmtRp(r.saldoAkhir||0)}</td></tr>)})}
    </tbody></table></div>}</div>)}
  </div>);
}
...
function MainApp({account:acc,onLogout,appData,setAppData,onSave}){
  ...
  <main style={S.main}>
    {tab==="dashboard"&&(<div>
      <div style={{background:"linear-gradient(135deg,#1e1e3a,#2a2a4a)",borderRadius:14,padding:"20px 24px",color:"#e0e0f0",marginBottom:18,border:"1px solid rgba(255,255,255,0.06)"}}><div style={{fontSize:16,fontWeight:600}}>Selamat datang, {acc.nama}</div><div style={{fontSize:11,opacity:.5,marginTop:2}}>Ringkasan Keuangan · Data tersimpan otomatis {I.cloud}</div></div>
      ...
      <div style={S.grid2}><div style={S.statCard}><div style={S.statLbl}>Karyawan: {eAkt} aktif</div><div style={{...S.statVal,color:"#b8b2ff"}}>{fmtRp(tGaji)}</div><div style={S.statMeta}>Total diterima/bulan</div></div><div style={{...S.statCard,borderTop:"3px solid #e67e22"}}><div style={S.statLbl}>Transaksi</div><div style={{display:"flex",justifyContent:"space-between",marginTop:6}}><div><div style={{fontSize:10,color:"#666"}}>Masuk</div><div style={{fontWeight:700,color:"#27ae60"}}>{fmtRp(tIn)}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:10,color:"#666"}}>Keluar</div><div style={{fontWeight:700,color:"#c0392b"}}>{fmtRp(tOut)}</div></div></div></div></div>
    </div>)}
...
export default function App(){
  ...
  if(loading)return(
    <div style={{fontFamily:"'Plus Jakarta Sans',sans-serif",minHeight:"100vh",background:"#0f0f1a",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}>
      <style>{gCSS}</style>
      <div style={{width:64,height:64,borderRadius:14,background:"rgba(255,255,255,0.06)",display:"flex",alignItems:"center",justifyContent:"center",border:"1px solid rgba(255,255,255,0.06)"}}><img src={LOGO_IMG} style={{width:48,height:"auto"}} alt="DRT"/></div>
      <div style={{color:"#8b8baa",fontSize:14,fontWeight:500}}>Memuat data...</div>
    </div>
  );
...
const S={
  shell:{fontFamily:"'Plus Jakarta Sans',sans-serif",display:"flex",minHeight:"100vh",background:"#0f0f1a",color:"#e0e0f0",justifyContent:"center"},
  side:{width:240,background:"linear-gradient(180deg,#12122a,#1a1a35)",color:"#fff",display:"flex",flexDirection:"column",position:"fixed",top:0,left:0,bottom:0,zIndex:100,transform:"translateX(-100%)",transition:"transform 0.25s ease",borderRight:"1px solid rgba(255,255,255,0.05)"},sideOpen:{transform:"translateX(0)"},sideOver:{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:99},sideBrand:{padding:"16px 14px",display:"flex",alignItems:"center",gap:8,borderBottom:"1px solid rgba(255,255,255,0.06)"},sideLogo:{width:36,height:36,borderRadius:8,background:"rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0},sideBT:{fontSize:13,fontWeight:700,color:"#e0e0f0"},sideBS:{fontSize:9,opacity:.4},sideX:{marginLeft:"auto",background:"none",border:"none",color:"rgba(255,255,255,0.3)",cursor:"pointer",padding:4},sideNav:{flex:1,padding:"10px 8px",display:"flex",flexDirection:"column",gap:2},sideItem:{display:"flex",alignItems:"center",gap:9,padding:"10px 12px",borderRadius:8,border:"none",background:"transparent",color:"rgba(255,255,255,0.5)",cursor:"pointer",fontSize:12,fontWeight:500,textAlign:"left",width:"100%"},sideItemA:{background:"rgba(108,99,255,0.15)",color:"#b8b2ff",fontWeight:600},sideBot:{padding:"10px 10px 14px",borderTop:"1px solid rgba(255,255,255,0.06)"},sideUser:{display:"flex",alignItems:"center",gap:8,marginBottom:8},sideAv:{width:30,height:30,borderRadius:"50%",background:"rgba(108,99,255,0.2)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:700,color:"#b8b2ff"},sideLogout:{display:"flex",alignItems:"center",gap:7,padding:"7px 10px",borderRadius:7,border:"none",background:"rgba(255,255,255,0.04)",color:"rgba(255,255,255,0.4)",cursor:"pointer",fontSize:11,width:"100%"},
  content:{width:"100%",maxWidth:780,display:"flex",flexDirection:"column",minHeight:"100vh",margin:"0 auto"},
  topbar:{background:"#1a1a2e",padding:"12px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,zIndex:50,borderBottom:"1px solid rgba(255,255,255,0.05)"},menuBtn:{background:"none",border:"none",cursor:"pointer",color:"#8b8baa",padding:3,display:"flex"},pageTitle:{fontSize:16,fontWeight:700,color:"#e0e0f0"},
  topClock:{display:"flex",flexDirection:"column",alignItems:"flex-end"},topTime:{fontSize:17,fontWeight:300,color:"#8b8baa",fontVariantNumeric:"tabular-nums"},topDate:{fontSize:10,color:"#888"},
  ...
  statCard:{background:"#1a1a2e",borderRadius:12,padding:"14px 16px",border:"1px solid rgba(255,255,255,0.05)"},statLbl:{fontSize:10,fontWeight:500,color:"#666",marginBottom:4},statVal:{fontSize:19,fontWeight:700,fontVariantNumeric:"tabular-nums"},statMeta:{fontSize:10,color:"#888",marginTop:3},
  card:{background:"#1a1a2e",borderRadius:14,padding:"18px 20px",border:"1px solid rgba(255,255,255,0.05)",marginBottom:12},cardHead:{fontSize:13,fontWeight:700,color:"#e0e0f0",marginBottom:12,paddingBottom:7,borderBottom:"1px solid rgba(255,255,255,0.06)"},
  progressOuter:{height:5,background:"#252540",borderRadius:3,overflow:"hidden"},progressInner:{height:"100%",borderRadius:3,transition:"width 0.4s"},
  lbl:{fontSize:10,fontWeight:600,color:"#666",display:"block",marginBottom:3},
  input:{padding:"9px 12px",fontSize:12,border:"1.5px solid rgba(255,255,255,0.08)",borderRadius:8,background:"#12122a",color:"#e0e0f0",width:"100%",fontFamily:"inherit"},
  loginInput:{padding:"10px 12px",fontSize:13,border:"1.5px solid rgba(255,255,255,0.08)",borderRadius:10,background:"#12122a",color:"#e0e0f0",width:"100%",fontFamily:"inherit"},
  select:{padding:"9px 12px",fontSize:12,border:"1.5px solid rgba(255,255,255,0.08)",borderRadius:8,background:"#12122a",color:"#e0e0f0",cursor:"pointer",fontFamily:"inherit",width:"100%"},
  btnPri:{padding:"9px 16px",fontSize:12,fontWeight:600,border:"none",borderRadius:8,background:"linear-gradient(135deg,#6c63ff,#4834d4)",color:"#fff",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:5,whiteSpace:"nowrap",boxShadow:"0 2px 8px rgba(108,99,255,0.2)"},btnSec:{padding:"9px 14px",fontSize:12,fontWeight:500,border:"1.5px solid rgba(255,255,255,0.1)",borderRadius:8,background:"transparent",color:"#8b8baa",cursor:"pointer"},btnGhost:{display:"flex",alignItems:"center",gap:4,padding:"7px 12px",fontSize:12,fontWeight:500,border:"1.5px solid rgba(255,255,255,0.1)",borderRadius:8,background:"transparent",color:"#b8b2ff",cursor:"pointer"},
  btnRet:{display:"flex",alignItems:"center",gap:3,padding:"5px 9px",fontSize:11,fontWeight:500,border:"1.5px solid #6c63ff",borderRadius:5,background:"rgba(108,99,255,0.1)",color:"#b8b2ff",cursor:"pointer",whiteSpace:"nowrap"},
  ...
  iconBtn:{background:"none",border:"none",cursor:"pointer",color:"#8b8baa",padding:3,display:"flex",borderRadius:3},
  linkBtn:{background:"none",border:"none",cursor:"pointer",color:"#b8b2ff",fontWeight:600,fontSize:12,textDecoration:"underline",textUnderlineOffset:2,padding:0,fontFamily:"inherit"},
  tableWrap:{overflowX:"auto"},table:{width:"100%",borderCollapse:"collapse",fontSize:11},th:{padding:"9px 10px",fontWeight:600,fontSize:9,color:"#666",borderBottom:"1px solid rgba(255,255,255,0.06)",textAlign:"center",whiteSpace:"nowrap",textTransform:"uppercase",letterSpacing:"0.5px"},td:{padding:"9px 10px",borderBottom:"1px solid rgba(255,255,255,0.04)",textAlign:"center",fontVariantNumeric:"tabular-nums",color:"#ccc"},
  badge:{padding:"2px 8px",borderRadius:16,fontSize:9,fontWeight:600,whiteSpace:"nowrap"},
  empty:{fontSize:12,color:"#888",fontStyle:"italic",padding:"20px 0",textAlign:"center"},
};
